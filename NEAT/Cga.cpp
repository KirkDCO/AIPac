...
//------------------------------------------------------------------------
vector<CNeuralNet*> Cga::Epoch(const vector<double> &FitnessScores, const vector<double> &GameScores)
{
  ...
  for (int gen=0; gen<m_vecGenomes.size(); gen++)
  {
    ...
    m_vecGenomes[gen].SetScore(GameScores[gen]);
  }
...
void Cga::SortAndRecord()
{
  ...
  {
//      fstream log;
//      log.open("storebestever.log",ios::out|ios::app);
//      m_vecGenomes[0].Write(log);
//      log << endl;
//      log.close();

    m_dBestEverFitness = m_vecGenomes[0].Fitness();

    m_BestEverGenome.DeletePhenotype(); //I assume this will take care of memory leaks - we'll see.
    m_BestEverGenome = m_vecGenomes[0];

    //store other characteristics of best genome
    m_dBestGenomeScore = m_BestEverGenome.GetScore();
    m_iBestGenomeLinks = m_BestEverGenome.GetNumLinks();
    m_iBestGenomeNeurons = m_BestEverGenome.NumNeurons();
  }
...
void Cga::StoreBestGenomes()
{
  ...
  for (int gen=0; gen<CParams::iNumBest; gen++)
  {
...
CNeuralNet* Cga::GetBestPhenotypeEver()
{
//    fstream log;
//    log.open("bestever.log", ios::out|ios::app);
//    m_BestEverGenome.Write(log);
//    log << endl;
//    log.close();
    return m_BestEverGenome.CreatePhenotype();
}
...
void Cga::ResetAndKill()
{
  ...
  {
//    vector<int> GenomesToKill = curSp->GetGenomeIDs(); //have to do this before ->purge() so I actually get a list of IDs
    curSp->Purge(); //seems odd that this purge is up here.  If I move it below, the species monitor does not behave.
                    //I put the GenomeID extraction above it to fix issues of losing members of the removed species
