...
vector<int> CSpecies::GetGenomeIDs()
{
    //return the IDs of all genomes in this species
    //used to facilitate killing off all genomes in a particular species
    vector<int> gIDs;
    gIDs.reserve(m_vecMembers.size()); //slight performance boost

    vector<CGenome*>::iterator it;
    for ( it = m_vecMembers.begin(); it != m_vecMembers.end(); it++)
        gIDs.push_back( (*it)->ID() );
